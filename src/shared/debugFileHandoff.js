...
function openHandoffDb() {
    return new Promise((resolve, reject) => {
        ...
        if (!indexedDb) {
            reject(new Error('Your browser does not support local IndexedDB storage. Please load files directly.'));
            return;
        }
        ...
        request.onerror = () => reject(request.error || new Error('Failed to open local IndexedDB storage.'));
...
export async function saveDebugFileHandoff(file, targetKind = getDebugFileKind(file)) {
    ...
    await new Promise((resolve, reject) => {
        ...
        transaction.onerror = () => reject(transaction.error || new Error('Failed to save file to IndexedDB.'));
        transaction.onabort = () => reject(transaction.error || new Error('IndexedDB save transaction aborted.'));
    }).finally(() => db.close());
    return record;
}
...
export async function consumeDebugFileHandoff(expectedKind = null) {
    ...
    await new Promise((resolve, reject) => {
        ...
        transaction.oncomplete = () => resolve();
        transaction.onerror = () => reject(transaction.error || new Error('Failed to read file from IndexedDB.'));
