...
function createRuntimeCanvas(width, height) {
    ...
    throw new Error('No Canvas available in the current environment');
}
...
async function getVideoContext(file) {
    ...
    if (!videoTrack) {
        input.dispose();
        throw new Error('No processable video track found in file');
    }
...
export async function removeGeminiVideoWatermark(file, options = {}) {
    ...
    if (!detection.isConfident && options.allowLowConfidence !== true) {
        throw new Error("Video watermark detection confidence is too low, export stopped. You can enable 'Force Export on Low Confidence' and retry.");
    }
    ...
    if (!canEncodeAvc) {
        throw new Error('Your browser does not support WebCodecs H.264/AVC encoding. Please use a modern version of Chrome or Edge.');
    }
    ...
    try {
        ...
        if (!target.buffer) {
            throw new Error('Video export failed: output buffer is empty');
        }
