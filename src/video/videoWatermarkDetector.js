...
function getVideoAlphaMap(size, options = {}) {
    ...
    if (!alphaSource) {
        throw new Error('Missing 96px Gemini alpha map, cannot generate video watermark template');
    }
...
export function detectDiamondVideoWatermarkFromFrames({
    ...
    if (!candidates.length) {
        throw new Error(`Video watermark candidate for ${width}x${height} is not supported yet`);
    }
